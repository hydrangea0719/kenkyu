...
for i in tmp1:
    for j in tmp2:
        filename = 'mai20' + i + '_' + j
        path_r = './dataset/' + filename + '.csv'
        path_w = './dataset_text/' + filename + '.txt'

        # 手作業編集済みのものを上書きされると悲しいのでバックアップを取る
        backname = path_w + '.bak'
        shutil.copy(path_w, backname)


        a_list = []

        with open(path_r, mode='r') as f_r:

            for row in csv.reader(f_r):
